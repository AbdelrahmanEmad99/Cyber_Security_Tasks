...
public class DES {
    ...
    private static String binaryToHex(String binary) {
        ...
        BigInteger bigInt = new BigInteger(binary, 2);// convert to decimal
        String hex = bigInt.toString(16).toUpperCase(); // convert to hexa
        // Ensure the hex string is padded to 16 digits (64 bits / 4)
        while (hex.length() < 16) {
            hex = "0" + hex;
        }
        return hex;
    ...
    private String xor(String a, String b) {
        ...
        for (int i = 0; i < a.length(); i++) {
            // XOR the bits and append the result ('0' ^ '0' = '0', '0' ^ '1' = '1', etc.)
            result.append(a.charAt(i) == b.charAt(i) ? '0' : '1'); // same =0 , diff = 1
        }
    ...
    private static String[] generateSubKeys(String keyBin) {
        ...
        String C = permutedKey.substring(0, 28); // 28 bits
        String D = permutedKey.substring(28); // 28 bits
    ...
    private String feistel(String R, String subKey) {
        ...
        for (int i = 0; i < 8; i++) {
            ...
            // Convert the S-box value (0 to 15) to a 4-bit binary string.
            substituted.append(intToBinaryString(sboxValue, 4));
        }

        // TODO: Apply permutation P to the substituted output
        String finalOutput = permute(substituted.toString(), P);
        return finalOutput;

    }
    private static String intToBinaryString(int num, int width) {
